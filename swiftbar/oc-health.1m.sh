...
def blanks(n): print("\n".join([""]*n))
if not p.exists(): blanks(4); sys.exit(0)
try: o = json.load(open(p))
except Exception: blanks(4); sys.exit(0)
...
: "${ROUTER_IP:=10.0.0.1}"; : "${NAS_IP:=10.0.0.6}"; : "${MUTINY_IP:=10.0.0.4}"; : "${MUTINY_SSH:=justin@10.0.0.4}"
