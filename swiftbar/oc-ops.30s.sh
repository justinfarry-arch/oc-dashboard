...
PYTHON="$(command -v python3)"; [ -x "$PYTHON" ] || PYTHON="/opt/homebrew/bin/python3"; [ -x "$PYTHON" ] || PYTHON="/usr/bin/python3"
...
def blanks(n): print("\n".join([""]*n))
if not p.exists(): blanks(9); sys.exit(0)
try: o = json.load(open(p))
except Exception: blanks(9); sys.exit(0)
S = o.get("IP allowlisting") or {}; O = o.get("other") or {}
pick = lambda *vals: next((v for v in vals if v not in (None,"",[])), "")
vals = [
...
# Fallbacks if config failed
: "${ROUTER_IP:=10.0.0.1}"; : "${NAS_IP:=10.0.0.6}"; : "${MUTINY_IP:=10.0.0.4}"; : "${TX_HOSTPORT_LOCAL:=10.0.0.4:9091}"; : "${MUTINY_SSH:=justin@10.0.0.4}"

# Debug (stderr so SwiftBar ignores it)
echo "DBG => R:${ROUTER_IP} N:${NAS_IP} S:${MUTINY_IP} TX:${TX_HOSTPORT_LOCAL} SSH:${MUTINY_SSH} URL:${DASH_URL}" >&2

# Checks: ICMP then TCP fallback
host_emoji() {
  local ip="$1"; shift
  [ -z "$ip" ] && { echo "🟥"; return; }
...
# Transmission active torrent count via SSH
